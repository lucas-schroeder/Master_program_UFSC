...
total_time = ceil(18*Mc/cs);
transient = ceil(10*Mc/cs);
...
% cossine sweep
sweep_time = 1:ceil(3*Mc/cs);          % timesteps
f_start = eps*(Dx/zeta);               % frequency at initial time
t_finish = ceil(3*Mc/cs);              % reference time
f_finish = 16e3*(Dx/zeta);             % frequency at reference time
sweep_aux = chirp(sweep_time, f_start, t_finish, f_finish);
sweep = zeros(1,total_time);
sweep(transient:transient+ceil(3*Mc/cs)-1) = sweep_aux;
plot(time,sweep);
...
%     figure(1)
%     surf(rho-1), view(2), shading flat, axis equal, caxis([-.05 .05]);
%     hold on;
...
writematrix([time' p_inlet_t' p_outlet_t'],"pressure_history_flow"+datestr(end_time, 'yyyy-mm-dd-HHMMSS')+".csv");
...
surf(ux), view(2), shading flat, axis equal, caxis([-0.1 0.1]), colorbar;
hold on;
plot(Mc-8*r,95:119, "xk");
plot(30*r,95:119, "xk");
plot(x_keypoints,y_keypoints,"k");
...
%% Plot Ux profile at x=200 latticeLength
figure(10)
% plot(ux(94:120,200),94:120);
plot(ux(94:120,200)./cs,94:120,"x");
title("u_x velocity profile at x=200");
xlabel("u_x/c_s");
...
Pin_f = fft(p_inlet_t(transient:total_time));
Pout_f = fft(p_outlet_t(transient:total_time));
L = length(p_outlet_t(transient:total_time));
