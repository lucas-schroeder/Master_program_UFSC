...
fprintf("2D channel - Without Flow\n")
start_time = now;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
total_time = ceil(14*250/cs); % 18 wavepass over the duct
transient = ceil(4*250/cs);
time = 0:1:(total_time-1);


% cossine sweep
sweep_duration = ceil(0.8*(total_time-transient));
...
ux_inlet = 0.1*cs*1.4; % there is a correction factor
uy_inlet = 0;
rho_inlet = 1;
...
writematrix([time' p_outlet_t' ux_outlet_t'],"pressure_list3_Lucas_Schroeder_1_2D_Flow_"+datestr(end_time, 'yyyy-mm-dd-HHMMSS')+".csv");

%% Surface plot last timestep
figure(4)
surf(ux), view(2), shading flat, axis equal, caxis([-0.02 0.1]), colorbar;
hold on;
plot(x_cylynder,y_cylinder,"k");
plot(x_buffer,y_buffer,"--k");
plot(x_inlet,y_inlet,"--k");
...
R1_2DChannel_Flow = [ka.' (Z./Z_0).' R.'];
save("R1_2DChannel_Flow.mat","R1_2DChannel_Flow");
