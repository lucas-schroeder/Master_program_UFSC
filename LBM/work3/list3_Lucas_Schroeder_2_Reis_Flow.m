...
fprintf("Axisymmetric Reis et al. Model - With Flow\n")
start_time = now;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
total_time = ceil(10*250/cs); % 15 wavepass over the duct
transient = ceil(3.5*250/cs);
time = 0:1:(total_time-1);


% cossine sweep
sweep_duration = ceil(0.9*(total_time-transient));
sweep_time = 1:sweep_duration;           % timesteps vector
f_start = eps;                           % frequency at initial time
t_finish = sweep_duration;               % reference time
ka_max = 2;                              % maximum Helmholtz number
...
ux_inlet = 0.1*cs*1.26; % FIX ME to include flow
uy_inlet = 0;
rho_inlet = 1;
...
for ta = 1 : total_time
    ...
    % Reis et al. (2007) Axisymmetric Condition    
    dUy_dy = gradient(uy,1,1);
    dUx_dy = gradient(ux,1,1);
    dUy_dx = gradient(uy,1,2);
    dUx_dx = gradient(ux,1,2);

    Q_xx = -(rho./(3*omega)).*(2.*dUx_dx);
    Q_yy = -(rho./(3*omega)).*(2.*dUy_dy);
    Q_xy = -(rho./(3*omega)).*(dUy_dx + dUx_dy);
    for i = 1:9
        h_1(:,:,i) = -W(i)*rho.*uy./R;
        
        t1_h2 = ((C_y(i).^2./2).*((ux.*dUy_dx)-(3.*uy.*omega.*Q_xx./2)-(3.*uy.*omega.*Q_yy)-(rho.*uy.^2./R)));
        t2_h2 =  -(C_x(i).*(((6.*nu./(6.*nu+1)).*Q_xy)+(rho.*dUy_dx./6)-(rho.*uy.*ux)));
        t3_h2 = ((C_y(i).*(1-12.*nu)).*((Q_yy./(2.*(1+6.*nu)))+ (rho.*uy./R)-(rho.*uy.^2)));
        h_2(:,:,i) = (W(i)*3./R).*(t1_h2 + t2_h2 + t3_h2);         
    end
...
writematrix([time' p_outlet_t' ux_outlet_t'],"pressure_list3_Lucas_Schroeder_2_Reis_Flow"+datestr(end_time, 'yyyy-mm-dd-HHMMSS')+".csv");
%%
figure(4)
...
%% FFT and Impedance
p_outlet_t2 = p_outlet_t(transient:end);
ux_outlet_t2 = ux_outlet_t(transient:end);
...
R2_Reis_Flow = [ka.' (Z./Z_0).' Ref.'];
save("R2_Reis_Flow.mat","R2_Reis_Flow");

%% Verify Velocity Profile
figure(10)
plot(ux(2:22,175)./cs,2:22,"-o");
title("u_x velocity profile at x=175");
xlabel("u_x/c_s");
ylabel("Lattice position, lattice units");
set(gca,"FontSize",20);
ylim([0 23]);
xlim([-0.01 0.2]);
grid on;

Mach_med = mean(ux(2:22,175))/cs
Mach_peak = max(ux(2:22,175))/cs
