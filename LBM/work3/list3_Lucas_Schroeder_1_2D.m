...
% cossine sweep
sweep_time = 1:ceil(total_time/4*3);     % timesteps
f_start = eps;                           % frequency at initial time
t_finish = sweep_time(end);              % reference time
ka_max = 2;
f_finish = cs*ka_max/(2*pi*r_a);         % frequency at reference time
...
sweep(1:length(sweep_time)) = sweep_aux.*exp_win;
...
for ta = 1 : total_time
    ...
    rho_outlet_t(ta) = mean(rho(2:20,250)-rho_l, 'all');
    ux_outlet_t(ta) = mean(ux(2:20,250), 'all');
...
% %     surf(ux), view(2), shading flat, axis equal;
%     xlim([1 Mc]);
%     ylim([1 Nr]);
%     % figure(2)
%     % plot(ux(150,:))
...
writematrix([time' p_outlet_t' ux_outlet_t'],"pressure_list3_Lucas_Schroeder_1_2D_"+datestr(end_time, 'yyyy-mm-dd-HHMMSS')+".csv");
%%
figure(4)
surf(ux), view(2), shading flat, axis equal, caxis([-0.01 0.01]), colorbar;
hold on;
plot(x_cylynder,y_cylinder,"k");
plot(x_buffer,y_buffer,"--k");
plot(x_inlet,y_inlet,"--k");
...
figure(8)
subplot(2,1,1)
plot(ka,real(Z./Z_0),'LineWidth',2);
...
%     ylim([0.4 1]);
    set(gca,"FontSize",16);
subplot(2,1,2)
plot(ka,imag(Z./Z_0),'LineWidth',2);
    xlabel("Frequency in Helmholtz number, ka");
    ylabel("Reactance");
    xlim([0 1.8]);
%     ylim([0.4 1]);
...
R1_2DChannel = [ka.' (Z./Z_0).' R.'];
save("R1_2DChannel.mat","R1_2DChannel");
