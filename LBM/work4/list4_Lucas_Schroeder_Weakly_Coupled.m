...
% plot(50:150,w+100,'k','LineWidth',2);
hold off;
...
% As the flow do not affect the hope position, let's calculate it first
for ta = 1:total_time
...
%         xlim([50 150]);
    ...
    w_history(ta,1:end) = w;
end

% save raw data
writematrix([time' w_history(:,51)],"list4_Lucas_Schroeder_weakly_coupled_"+datestr(now, 'yyyy-mm-dd-HHMMSS')+".csv");
...
for ta = 1 : total_time
    ...
    for iDir = 1:8
        [dir(iDir).dist, dir(iDir).col, dir(iDir).line] = crossing(Nr,Mc,iDir, 50:1:150, w_history(ta,1:end)+100);
    end
    ...
    if and(mod(ta,100)==0, real_time_plot)
        ...
        plot(50:150,w_history(ta,1:end)+100,'k','LineWidth',2); hold off;
        colormap jet;
%         xlim([1 Mc]);
%         ylim([1 Nr]);
        ylim([90 110]);
...
save("last_timestep_weakly_coupled.mat","f");
